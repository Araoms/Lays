...
layui.define(['jquery', 'layer','contextMenu','form'], function (exports) {
        ...
        register: function () {
            ...
            if (layim) {
                ...
                layim.on('sign', function (value) {
                    $.post(g_site_url+'index.php?chat/user/ajaxpoplogin&action=change_sign', {sign: value}, function (data) {
    ...
    var im = {
        ...
        initListener: function (user,pwd) { //初始化监听
            ...
            conn.listen({
                ...
                onClosed: function ( message ) {
                    ...
                    //   ,closeBtn: 0
                    // }, function(){
                    //     window.location.href = g_site_url+"index.php?chat/user/ajaxpoplogin&action=login";
                    // });
                },         //连接关闭回调
                ...
                onPresence: function ( message ) {//监听对方的添加或者删除好友请求，并做相应的处理。
                    if (message.type == 'unsubscribe') {
                        ...
                        if (message.to == cachedata.mine.id && message.status =='Success') {
                            im.audio('新'); 
                            $.get(g_site_url+'index.php?chat/user/ajaxpoplogin&action=subscribed',{memberIdx:message.from},function(res){
                                var data = eval('(' + res + ')');
                                conf.layim.addList({
                                    type: 'friend' //列表类型，只支持friend和group两种
                                    ,avatar:  uploadsPerson+message.from +'.jpg' //好友头像
                        ...
                        console.log(message);
                        $.get(g_site_url+'index.php?chat/user/ajaxpoplogin&action=add_admin_msg', {from: message.from,adminGroup: message.to,to: message.gid}, function (res) {
                            // var data = eval('(' + res + ')');
                            // if (data.code == 0) {                             
                        ...
                        // $.get(g_site_url+'index.php?chat/user/ajaxpoplogin&action=get_one_user_data',{memberIdx:message.mid},function(res){
                            // var data = eval('(' + res + ')');
                            // conf.layim.addList({
                            //     type: 'group' //列表类型，只支持friend和group两种
                            //     ,avatar: uploadsPerson+message.mid +'.jpg' //好友头像
                        ...
                        im.audio('新');
                        var default_avatar = uploadsPerson+'empty1.jpg';
                        var avatar = uploadsPerson+message.from+'.jpg';
                        var options = {
                        ...
                        $.get(g_site_url+'index.php?chat/user/ajaxpoplogin&action=get_one_group_data',{groupIdx:message.from},function(res){
                            var data = eval('(' + res + ')');
                            layer.open({
                              type: 1,
                              shade: false,
                ...
                onRoster: function ( message ) {
                    ...
                    if (message[0].subscription == 'to' && message[0].ask == 'subscribe') {
                        $.get(g_site_url+'index.php?chat/user/ajaxpoplogin&action=get_one_user_data',{memberIdx:message[0].name},function(res){
                            var data = eval('(' + res + ')');
        ...
        defineMessage: function (message,msgType) {
            ...
            var data = {mine: false,cid: 0,username:message.ext.username,avatar:uploadsPerson+message.from+".jpg",content:msg,id:id,fromid: message.from,timestamp:timestamp,type:type}
            ...
            msg.set({
                ...
                success: function (id, serverMsgId) {//发送成功则记录信息到服务器
                    ...
                    if ((data.to.cmd && sendData.sysLog) || data.to.cmd == 0) {
                        $.get(g_site_url+'index.php?chat/user/ajaxpoplogin&action=addChatLog', sendData, function (res) {
                            var data = eval('(' + res + ')');
                            if (data.code != 0) {
                                console.log('消息发送失败');
                            }
        ...
        removeFriends: function (username) {
            conn.removeRoster({
                ...
                success: function () {  // 删除成功
                    $.get(g_site_url+'index.php?chat/user/ajaxpoplogin&action=removeFriends', {friend_id: username}, function (res) {
                        var data = eval('(' + res + ')');
                        if (data.code == 0) {
                            var index = layer.open();
                            layer.close(index);
        ...
        leaveGroupBySelf: function (to,username,roomId) {
            $.get(g_site_url+'index.php?chat/user/ajaxpoplogin&action=leaveGroup', {groupIdx:roomId,memberIdx:to}, function (res) {
                var data = eval('(' + res + ')');
                if (data.code == 0) {
                    var option = {
                        to: to,
        ...
        addFriendGroup:function(othis,type){
            ...
            var avatar = uploadsPerson+uid+'.jpg';
            var isAdd = false;
            if (type == 'friend') {
                var default_avatar = uploadsPerson+'empty2.jpg';
            ...
            parent.layui.layim.add({//弹出添加好友对话框
                ...
                ,submit: function(group,remark,index){//确认发送添加请求
                    if (type == 'friend') {
                        $.get(g_site_url+'index.php?chat/user/ajaxpoplogin&action=add_msg', {to: uid,msgType:1,remark:remark,mygroupIdx:group}, function (res) {
                            var data = eval('(' + res + ')');
                            if (data.code == 0) {
                                conn.subscribe({
                                    to: uid,
                        ...
                        var options = {
                            ...
                            success: function(resp) {
                                if (approval == '1') {
                                    $.get(g_site_url+'index.php?chat/user/ajaxpoplogin&action=add_msg', {to: uid,msgType:3,remark:remark}, function (res) {
                                        var data = eval('(' + res + ')');
                                        if (data.code == 0) {                        
                ...
                if (msgType == 2) {
                    ...
                    conf.layim.setFriendGroup({
                        ...
                        , submit: function (group, index) { 
                            $.get(g_site_url+'index.php?chat/user/ajaxpoplogin&action=modify_msg', {msgIdx: msgIdx,msgType:msgType,status:status,mygroupIdx:group,friendIdx:uid}, function (res) {
                                var data = eval('(' + res + ')');
                                if (data.code == 0) {
                                    //将好友 追加到主面板
                                    conf.layim.addList({
                    ...
                    var default_avatar = uploadsPerson+'empty1.jpg';
                    $.get(g_site_url+'index.php?chat/user/ajaxpoplogin&action=modify_msg', {msgIdx: msgIdx,msgType:msgType,status:status}, function (res) {
                        var data = eval('(' + res + ')');
                        if (data.code == 0) {
                            var options = {
                                    applicant: uid,
        ...
        commitGroupInfo: function(othis,data){
            ...
            $.get(g_site_url+'index.php?chat/user/ajaxpoplogin&action=userMaxGroupNumber', {}, function(res){
                    ...
                    var options = {
                        ...
                        success: function (respData) {
                            if (respData.data.groupid) {
                                $.get(g_site_url+'index.php?chat/user/ajaxpoplogin&action=commitGroupInfo', {groupIdx:respData.data.groupid,groupName: data.groupName,des:data.des,number:data.number,approval:data.approval}, function(respdata){
                                    var res = eval('(' + respdata + ')');
                                    if(res.code == 0){
                                        //将群 追加到主面板
                                        var avatar = uploadsPerson+respData.data.groupid+'.jpg';
        ...
        userStatus: function(data){
            if (data.id) {
                $.get(g_site_url+'index.php?chat/user/ajaxpoplogin&action=userStatus', {id:data.id}, function (res) {
                    var data = eval('(' + res + ')');
                    if (data.code == 0) {  
                        if (data.data == 'online') {
        ...
        addMyGroup: function(){//新增分组
            $.get(g_site_url+'index.php?chat/user/ajaxpoplogin&action=addMyGroup', {}, function (res) {
                var data = eval('(' + res + ')');
                if (data.code == 0) {
        ...
        delMyGroup: function(groupidx){//删除分组
            $.get(g_site_url+'index.php?chat/user/ajaxpoplogin&action=delMyGroup', {mygroupIdx:groupidx}, function (res) {
                var data = eval('(' + res + ')');
                if (data.code == 0) {
                        ...
                        if(groupList === groupidx){//要删除的分组
                            if (group.eq(j).find('ul span').hasClass('layim-null')) {//删除的分组下没有好友
                                ...
                                for (var i = 0; i < number; i++) {
                                    ...
                                    var avatar = uploadsPerson+friend_id+'.jpg';
                                    var default_avatar = uploadsPerson+'empty2.jpg';
        ...
        setAdmin: function(othis){
            ...
            var options = {
                    ...
                    username: username,
                    success: function(resp) {
                        $.get(g_site_url+'index.php?chat/user/ajaxpoplogin&action=setAdmin', {groupidx:groupidx,memberIdx:username,type:2}, function (res) {
                            var admin = eval('(' + res + ')');
                            if (admin.code == 0) { 
        ...
        removeAdmin: function(othis){
            ...
            var options = {
                    ...
                    username: username,
                    success: function(resp) {
                        $.get(g_site_url+'index.php?chat/user/ajaxpoplogin&action=setAdmin', {groupidx:groupidx,memberIdx:username,type:3}, function (res) {
                            var admin = eval('(' + res + ')');
                            if (admin.code == 0) { 
                                $("ul[data-groupidx="+groupidx+"] #"+username).remove();
        ...
        editGroupNickName: function(othis){
            ...
            layer.prompt({title: '请输入群名片，并确认', formType: 0,value: name[0]}, function(nickName, index){
                $.get(g_site_url+'index.php?chat/user/ajaxpoplogin&action=editGroupNickName',{nickName:nickName,memberIdx:memberIdx,groupIdx:groupIdx},function(res){
                    var data = eval('(' + res + ')');
                    if (data.code == 0) {
        ...
        leaveGroup: function(groupIdx,list,username){//list为数组
            $.get(g_site_url+'index.php?chat/user/ajaxpoplogin&action=leaveGroup',{list:list,groupIdx:groupIdx},function(res){
                var data = eval('(' + res + ')');
                if (data.code == 0) {
                    var options = {
                        roomId: groupIdx,
            ...
            return data =  {
                            ...
                            callback: function(ele) {
                                ...
                                layer.prompt({title: '修改备注姓名', formType: 0,value: friend_name}, function(nickName, index){
                                    $.get(g_site_url+'index.php?chat/user/ajaxpoplogin&action=editNickName',{nickName:nickName,friend_id:friend_id},function(res){
                                        var data = eval('(' + res + ')');
                                        if (data.code == 0) {                                              
            ...
            return data = {
                            ...
                            callback: function(ele) {
                                ...
                                item.hover(function() {
                                    ...
                                    $.get(g_site_url+'index.php?chat/user/ajaxpoplogin&action=moveFriend',{friend_id:friend_id,groupidx:groupidx},function(res){
                                        var data = eval('(' + res + ')');
                                        if (data.code == 0) {
                                            conf.layim.removeList({//将好友从之前分组除去
                                                type: 'friend' 
        ...
        menuRemove: function(){
            return data = {
                        ...
                        callback: function(ele) {
                            ...
                            layer.confirm('删除后对方将从你的好友列表消失，且以后不会再接收此人的会话消息。<div class="layui-layim-list"><li layim-event="chat" data-type="friend" data-index="0">' +
                                '<img src="'+uploadsPerson+friend_id+'.jpg"><span>'+username+'</span><p>'+sign+'</p></li></div>', {
                                btn: ['确定','取消'], //按钮
            ...
            return  data =  {
                        ...
                        callback: function(ele) {
                            ...
                            layer.prompt({title: '请输入分组名，并确认', formType: 0,value: groupName}, function(mygroupName, index){
                                if (mygroupName) {
                                    $.get(g_site_url+'index.php?chat/user/ajaxpoplogin&action=editGroupName',{mygroupName:mygroupName,mygroupIdx:mygroupIdx},function(res){
                                        var data = eval('(' + res + ')');
                                        if (data.code == 0) {
            ...
            return  data =  {
                                ...
                                callback: function(ele) {
                                    ...
                                    item.hover(function() {
                                        ...
                                        $.get(g_site_url+'index.php?chat/user/ajaxpoplogin&action=groupMemberGag',{gagTime:gagTime,groupidx:groupidx,friend_id:friend_id},function(resp){
                                            var data = eval('(' + resp + ')');
                                            if (data.code == 0) {
        ...
        menuLiftGroupMemberGag: function(){
            return  data =  {
                                ...
                                callback: function(ele) {
                                    ...
                                    friend_name = ele.parent().data('name');
                                    groupidx = ele.parent().data('groupidx');
                                    $.get(g_site_url+'index.php?chat/user/ajaxpoplogin&action=liftGroupMemberGag',{groupidx:groupidx,friend_id:friend_id},function(resp){
                                        var data = eval('(' + resp + ')');
                                        if (data.code == 0) {
                                            var res = {mine: {
                                                            content: '0',
