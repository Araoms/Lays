                ...
                <!--{loop $expertlist $expert}-->
                    <li class="index-userwrap">
                        <a href="{url user/space/$expert['uid']}" target="_self" class="avatar">
                            <img src="{$expert['avatar']}" class="avatarPic lazy" alt="katray" title="katray" style="display: inline;">
                            <span class="icons">
...
$expert['author_has_vertify']!=false}<i class="fa fa-vimeo v_person   " data-toggle="tooltip" data-placement="right" title="" data-original-title="认证用户"></i>{/if}</a>
                            <p class="intro"><span>{$expert['signature']}</span></p>
...
<!-- 专家申请入口 -->
<a {if $user['uid']==0} href="javascript:login()" {else} href="{url user/vertify}" {/if}  class="ab_expert_block animated  rubberBand "><div class="icon_expert" ></div><span>{eval echo trim($setting['vertify_gerentip']);}认证</span></a>
