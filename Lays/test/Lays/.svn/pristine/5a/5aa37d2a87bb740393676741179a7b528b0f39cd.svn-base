...
class Update extends CI_Controller {
	...
	function index() {
		...
		$sql="CREATE TABLE IF NOT EXISTS `" . $this->db->dbprefix . "user_invateanswer` (
  `Id` int(11) NOT NULL AUTO_INCREMENT,
  `uid` int(11) DEFAULT NULL COMMENT '用户uid',
  `qid` int(11) DEFAULT NULL COMMENT '问题id',
  `invatetime` int(11) DEFAULT NULL COMMENT '邀请时间',
  `cid` int(11) DEFAULT NULL COMMENT '邀请回答话题id',
    `invateuid` int(11) DEFAULT NULL COMMENT '邀请人的uid',
    `state` varchar(255) DEFAULT '1' COMMENT '用户邀请状态，默认是1，0表示取消邀请',
  PRIMARY KEY (`Id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='用户邀请回答表';
		";
		if($this->db->query ( $sql )){
			echo ' 更新成功:新增user_invateanswer邀请回答表<br>';
		}
  ...
  PRIMARY KEY (`id`)
		...
		echo ' 更新成功:新增article_comment文章评论回复表<br>';
		    //---邀请人的邀请码
		//-----
		$sql_class1 = "alter table " . $this->db->dbprefix . "message add COLUMN typename VARCHAR(200)  DEFAULT NULL;";
		$this->db->query ( $sql_class1 );
		echo ' 更新成功:更新message表，增加typename消息类型<br>';
...
ADD  `wburl` VARCHAR( 200 ) NOT NULL";
		...
		try {
			...
			$groupid = 6;
			$youkeregular_code = ',question/answer,user/getpass,index/index,index/default,topic/userxinzhi,user/space_attention,user/space_ask,user/space_answer,user/space,tags/default,tags/index,tags/view,tags/question,tags/article,tags/all,tags/ajaxsearch,new/question,new/default,newpage/index,newpage/catname,newpage/maketag,topic/weeklist,topic/default,topic/catlist,topic/hotlist,api_user/bindregisterapi,api_user/registerapi,user/register,user/resetpass,user/getpass,topic/getone,';
			$group = $this->usergroup_model->get ( $groupid );
			if (! strstr ( $group ['regulars'], $youkeregular_code )) {
				
				$tmp = $group ['regulars'] . $youkeregular_code;
...
CREATE TABLE IF NOT EXISTS `" . $this->db->dbprefix . "topicclass` (
  ...
  PRIMARY KEY (`id`)
	...
	function config_edit() {
		...
		$config .= "define('Lays_CHARSET', '$version');\r\n";
		$config .= "define('Lays_RELEASE', '$versiondate');\r\n";
		...
		writetofile ( APPPATH . 'config'  . '/database.php', $strdata );

	}
	function config_webconfig(){
		$webconfig = "<?php \r\n";
		$webconfig .= "defined('BASEPATH') OR exit('No direct script access allowed');\r\n";
		if (! file_exists ( $file_path = APPPATH . 'config' . DIRECTORY_SEPARATOR . ENVIRONMENT . DIRECTORY_SEPARATOR . 'config.php' ) && ! file_exists ( $file_path = APPPATH . 'config' . DIRECTORY_SEPARATOR . 'config.php' )) {
			show_error ( 'The configuration file config.php does not exist.' );
		}
		...
		foreach ($config as $key=>$val){
			if(is_bool($val)||is_int($val)){
				if(is_bool($val)){
					if($val){
						$webconfig .="$"."config['".$key."']=TRUE;\r\n";
					}else{
						$webconfig .="$"."config['".$key."']=FALSE;\r\n";
					}
					
				}else{
					$webconfig .="$"."config['".$key."']=$val;\r\n";
				}
		...
		$url =SITE_URL ; //后续修改成https后还需将https切换成https
		$mobileurl=$config['mobile_domain'] ? trim($config['mobile_domain'] ):'';
		$dirname=$config['dir_name'] ? trim($config['dir_name'] ):'';
		$_webconfig = "<?php \r\n";
		$_webconfig .="$"."config['base_url']='".  "$url';//默认网站配置\r\n";
		$_webconfig .="$"."config['dir_name']='".  "$dirname';//如果是二级目录安装，这里是二级目录名称\r\n";
		$webstrdata = $_webconfig ."$"."config['mobile_domain']='".$mobileurl."';////移动端网站可配置,/结尾 \r\n?>";
		$fp = fopen(APPPATH . 'config/webconfig.php', 'w');
		fwrite($fp, $webstrdata);
		fclose($fp);
		unset($config['base_url']);
		$webconfig .="include_once 'webconfig.php';\r\n";
		$strdata =$webconfig."\r\n?>";
		writetofile ( APPPATH . 'config'  . '/config.php', $strdata );
	}
