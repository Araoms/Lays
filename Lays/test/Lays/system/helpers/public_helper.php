 ...
 *  类似表单的提交 curl
 */
 function http_curl_form($url, $params = false, $ispost = 0, $https = 0)
{
    //检测是否支持cURL
    ...
    var_dump($response,$url.'?'.$params);die;
    if ($response === FALSE) {
...
/***
  * 发送短信、
  * $Mobile   string 手机号  多个以 , 隔开
  * $Content  string 发送内容
  * $message_setting   setting   短信配置 key
  */
function sendsms($Mobile,$Content,$message_setting) {
    if(!$message_setting){
        return '短信未配置';
    }
    $message_setting = unserialize($message_setting);
    if(!$message_setting['message_setting_status']){
        return '发送短信未启用';
    }
    $sendUrl = $message_setting['domain'].'sdk2/BatchSend2.aspx'; // 短信接口的URL
    $Content_prefix = '【魔丸科技】';
    $message [] = array ();
    $smsConf = array (
        'CorpID' => $message_setting['CorpID'], // 您申请的APPKEY
        'Pwd' => $message_setting['Pwd'],       // 您申请的密码
        'Mobile' => $Mobile, // 发送的手机号码，多个以 , 隔开
        'Content' =>urlencode($Content.$Content_prefix),
        'Cell' => ''
    ); // 您设置的模板变量，根据实际情况修改

    $content = http_curl_form ($sendUrl,$smsConf); // 请求发送短信
...
if (! function_exists ( 'updateinfo' )) {
    ...
    function updateinfo($user) {
        ...
        $pars = array (
            ...
            'siteurl' => $siturl,
            'email' => $user ['email'],
            'version' => Lays_CHARSET,
            'release' => Lays_RELEASE,
            'phpos' => PHP_OS,
        ...
        return 'https://www.lays.vip/updatesite.php?' . $data;
    }
}
/*
