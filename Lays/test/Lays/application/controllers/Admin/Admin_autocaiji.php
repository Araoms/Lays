...
class Admin_autocaiji extends CI_Controller {
...
CREATE TABLE IF NOT EXISTS `" .  $this->db->dbprefix . "autocaiji` (
  ...
  PRIMARY KEY (`id`)
	...
	function index() {
	    ...
	    if($submit){
            $caijitype = $this->input->post('caijitype');
            $category1 = $this->input->post('category1');
            $category2 = $this->input->post('category2');
            $category3 = $this->input->post('category3');
        }else{
            $caijitype = $this->uri->rsegments[3];
            $category1 = $this->uri->rsegments[4];
            $category2 = $this->uri->rsegments[5];
            $category3 = $this->uri->rsegments[6];
        }
        $caijitype = isset($caijitype)?$caijitype:0;
        $where = ' caijitype ='.$caijitype;
        if($category1){
            $where .=  ' AND  category1 ='.$category1;
        }
        if($category2){
            $where .=  ' AND  category2 ='.$category2;
        }
        if($category3){
            $where .=  ' AND  category3 ='.$category3;
        }
		...
		if($submit){
            ...
            @$page = max ( 1, intval ( $this->uri->segment ( 7 ) ) );
        }
		$pagesize = 30;
		$startindex = ($page - 1) * $pagesize;
		$caijilist = array ();
		$query = $this->db->query (
		              "select * from " .  $this->db->dbprefix . "autocaiji WHERE $where order by id desc limit $startindex,$pagesize" );
		...
		$rownum = returnarraynum ( $this->db->query ( getwheresql ( "autocaiji", $where, $this->db->dbprefix ) )->row_array () );
		...
		$departstr = page ( $rownum, $pagesize, $page, "admin_autocaiji/default/$caijitype/$category1/$category2/$category3" );
	...
	function fillter($str) {
		...
		$str = preg_replace ( "/<(\!.*?)>/si", "\r\n", $str ); //过滤DOCTYPE
		$str = preg_replace ( "/<(\/?html.*?)>/si", "\r\n", $str ); //过滤html标签
		$str = preg_replace ( "/<(\/?br.*?)>/si", "\r\n", $str ); //过滤br标签
		$str = preg_replace ( "/<(\/?head.*?)>/si", "\r\n", $str ); //过滤head标签
		$str = preg_replace ( "/<(\/?meta.*?)>/si", "\r\n", $str ); //过滤meta标签
		$str = preg_replace ( "/<(\/?body.*?)>/si", "\r\n", $str ); //过滤body标签
		$str = preg_replace ( "/<(\/?link.*?)>/si", "\r\n", $str ); //过滤link标签
		$str = preg_replace ( "/<(\/?form.*?)>/si", "\r\n", $str ); //过滤form标签
		$str = preg_replace ( "/cookie/si", "COOKIE", $str ); //过滤COOKIE标签
		$str = preg_replace ( "/<(applet.*?)>(.*?)<(\/applet.*?)>/si", "\r\n", $str ); //过滤applet标签
		$str = preg_replace ( "/<(\/?applet.*?)>/si", "\r", $str ); //过滤applet标签
		$str = preg_replace ( "/<(style.*?)>(.*?)<(\/style.*?)>/si", "\r\n", $str ); //过滤style标签
		$str = preg_replace ( "/<(\/?style.*?)>/si", "\r\n", $str ); //过滤style标签
		$str = preg_replace ( "/<(title.*?)>(.*?)<(\/title.*?)>/si", "\r\n", $str ); //过滤title标签
		$str = preg_replace ( "/<(\/?title.*?)>/si", "\r\n", $str ); //过滤title标签
		$str = preg_replace ( "/<(object.*?)>(.*?)<(\/object.*?)>/si", "\r\n", $str ); //过滤object标签
		$str = preg_replace ( "/<(\/?objec.*?)>/si", "\r\n", $str ); //过滤object标签
		$str = preg_replace ( "/<(noframes.*?)>(.*?)<(\/noframes.*?)>/si", "\r\n", $str ); //过滤noframes标签
		$str = preg_replace ( "/<(\/?noframes.*?)>/si", "\r\n", $str ); //过滤noframes标签
		$str = preg_replace ( "/<(i?frame.*?)>(.*?)<(\/i?frame.*?)>/si", "\r\n", $str ); //过滤frame标签
		$str = preg_replace ( "/<(\/?i?frame.*?)>/si", "\r\n", $str ); //过滤frame标签
		$str = preg_replace ( "/<(script.*?)>(.*?)<(\/script.*?)>/si", "\r\n", $str ); //过滤script标签
		$str = preg_replace ( "/<(\/?script.*?)>/si", "\\r\n", $str ); //过滤script标签
		...
		$str = preg_replace ( "@<iframe(.*?)</iframe>@is", "\r\n", $str );
	...
	/**
      ...
      *  过滤掉 指定的敏感字
      */
	public function text($str,$pregAll){
	    if($pregAll){
            $pregAll = explode("\n",$pregAll);
            foreach ($pregAll as $key=>$val){
                if($val){
        ...
        $pattern_1  =  '/(id=)/';
        $pattern_2  =  '/(class=)/';
        $str =  preg_replace ( $pattern_1 ,  "" ,  $str );
        $str = preg_replace ( $pattern_2 ,  "" ,  $str );
	...
	function getarticle() {
		...
		$content = $this->input->post ("content"); //过滤指定内容
		...
		try {
            ...
            $result ['neirong'] = $this->text($result ['neirong'],$content);
...
// 			}
	...
	function ajaxcaijiwz() {
		...
		$ckabox = $this->input->post ("ckabox"); //采集过滤超链接
		$imgckabox = $this->input->post ("imgckabox"); //超级过滤图片
		$content_ = $this->input->post ("content"); //过滤掉不需要的内容
		...
		try {
			...
			if ($article != null) {
				...
				$url = 'http://www.lays.vip/data/attach/topic/topic6DYxVY.jpg';
