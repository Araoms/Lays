...
class Question extends CI_Controller {
	...
	function voice() {
		...
		$qid = intval ( $this->uri->segment ( 3 ) ); // 接收qid参数
		$aid = intval ( $this->uri->segment ( 4 ) ); // 接收aid参数
		...
		include template ( 'voice' );
	}
	/* 上传媒体文件 */
	function postmedia() {
		$useragent = $_SERVER ['HTTP_USER_AGENT'];
		if (! strstr ( $useragent, 'MicroMessenger' )) {
			$message ['message'] = '微信端操作.';
			echo json_encode ( $message );
			exit ();
		}
		if ($this->user ['uid'] == 0) {
			
			$message ['message'] = '游客无权操作，请登录！';
			echo json_encode ( $message );
			exit ();
		}
		...
		$voicetime = $this->input->post ( 'voicetime' );
		// 只允许专家回答问题
		...
		$content = '语音回答';
		$media_mp3file = '';
		if ($this->setting ['mobile_localyuyin'] == 1) {
			$media_mp3file = $this->downloadmedia ( $media_id );
		}
		$this->answer_model->addvoice ( $qid, $title, $content, $status, $chakanjine, $voicetime, $media_id, $openid, $media_mp3file );
		// 回答问题，添加积分
		...
		if ($model != null) {
			if ($model ['touid'] == $touid) {
				...
				$this->depositmoney_model->update ( $quid, 'eqid', $question ['id'] ); // 更新托管记录
			} else {
			}
		}
		...
		// 极光推送
		jpushmsg ( "您的回答有人偷看了", $question ['authorid'], "回答被偷看", $url );
		...
		if ($wx ['appsecret'] != '' && $wx ['appsecret'] != null && $wx ['winxintype'] != 2) {
			...
			if ($quser ['openid'] != '' && $quser ['openid'] != null) {
				...
				if (! $this->setting ['weixin_tpl_huida']) {
					...
					$returnmesage = $jssdk->sendanswertpltouser ( $quser ['openid'], $this->setting ['weixin_tpl_huida'], $url, "您好，您的在线咨询已经有网友回答，请及时查看回复并选择最佳答案。", $title, $description, $question ['answers'] . "人", "点击查看详情，满意回复请选择最佳答案" );
				}
			}
		}
	...
	function downloadmedia($media_id) {
		...
		$path = FCPATH . "data/weixinrecord/"; // 保存路径，相对当前文件的路径
		if (! is_dir ( $path )) {
			mkdir ( $path );
		}
		...
		$file_name = "wxupload_" . time () . rand ( 1111, 9999 );
		$amrfilename = $file_name . ".amr";
		$mp3 = $file_name . '.mp3';
		ob_start ();
		readfile ( $url );
		$img = ob_get_contents ();
		ob_end_clean ();
		$size = strlen ( $img );
		$fp = fopen ( $path . $amrfilename, 'a' );
		fwrite ( $fp, $img );
		fclose ( $fp );
		
		if (file_exists ( $path . $mp3 ) == true) {
			// runlog('mediaurl', 'ggggg');
			
			$message ['message'] = '已经存在相同的语音';
			echo json_encode ( $message );
			exit ();
		} else {
			if (file_exists ( $path . $amrfilename ) == false) {
				$message ['message'] = $amrfilename . '文件不存在';
				echo json_encode ( $message );
				
				// runlog('mediaurl', $amrfilename.'文件不存在');
				
				exit ();
			}
			// runlog('mediaurl', 'ttttt');
			$command = "/usr/local/bin/ffmpeg -i $path$amrfilename $path$mp3";
			// runlog('mediaurl', $command);
	...
	function ajaxanswer() {
		...
		sendmutiemail ( $emails, $subject, $mymessage );
		...
		$wx = $this->fromcache ( 'cweixin' );
		// 极光推送
		jpushmsg ( "您的提问[$title]有新的回答", $question ['authorid'], "您的问题有新的回答", $url );
		...
		if ($qid > 0) {
			...
			if ($askfromuid) {
				...
				if ($wx ['appsecret'] != '' && $wx ['appsecret'] != null && $wx ['winxintype'] != 2) {
					...
					if ($fromuser ['openid'] != '' && $fromuser ['openid'] != null) {
						$url = $question_url; // SITE_URL . $this->setting ['seo_prefix'] . $viewurl . $this->setting ['seo_suffix'];
						$text = "问题求助:(" . $title . ")，<a href='$url'>请点击查看详情!</a>";
						
						$returnmesage = $jssdk->sendtexttouser ( $fromuser ['openid'], $text );
					}
				}
		...
		if (strstr ( $useragent, 'MicroMessenger' ) && $wx ['appsecret'] != '' && $wx ['appsecret'] != null && $wx ['winxintype'] != 2) {
			...
			if ($this->setting ['openwxpay'] == 1) {
				...
				if (is_mobile ()) {
					...
					$baid = $this->input->post ( 'aid' );
					$authorid = $this->input->post ( 'authorid' );
					// ②、统一下单
					$t5 = rand ( 111111111, 999999999 );
					$input = new WxPayUnifiedOrder ();
					$shangjin = doubleval ( $this->input->post ( 'jine' ) );
					if ($shangjin == 0) {
						$shangjin = 10;
					} else {
						$shangjin = $shangjin * 100;
					}
					
					$input->SetBody ( "wxpay" );
					$input->SetAttach ( "aid_" . $baid . "_" . $authorid . "_$shangjin" );
					$input->SetOut_trade_no ( WxPayConfig::MCHID . date ( "YmdHis" ) );
					$input->SetTotal_fee ( "$shangjin" );
					$input->SetTime_start ( date ( "YmdHis" ) );
					// $input->SetTime_expire(date("YmdHis", time() + 600));
					$input->SetGoods_tag ( "tid_" . $baid . "_" . $authorid . "_$shangjin" );
					$input->SetNotify_url ( WxPayConfig::Notify_Url );
					$input->SetTrade_type ( "JSAPI" );
					$input->SetOpenid ( $openId );
					
					$order = WxPayApi::unifiedOrder ( $input );
					// runlog('answerpay',json_encode($order) );
					// var_dump($order);exit();
		...
		if (strstr ( $useragent, 'MicroMessenger' ) && $wx ['appsecret'] != '' && $wx ['appsecret'] != null) {
			...
			if ($this->setting ['openwxpay'] == 1) {
				...
				if ($openId == '' || $openId == null) {
					...
					if ($arr_query ['code'] != '' && $arr_query != null) {
						$url = SITE_URL . '?' . $this->getUrlQuery ( $arr_query );
						header ( "Location:$url" );
					}
				}
			}
	...
	function view() {
		...
		if ($this->config->item ( 'go_app' )) {
			if (strstr ( $useragent, $this->setting ['app_useragnet'] )) {
				$appurl = SITE_URL . $this->config->item ( 'go_app_dirname' ) . "question?id=$qid&comefrom=app";
				header ( "location:$appurl" );
			}
		}
		if ($this->config->item ( 'go_weixin' )) {
			if (strstr ( $useragent, 'MicroMessenger' )) {
				$appurl = SITE_URL . $this->config->item ( 'go_weixin_dirname' ) . "question?id=$qid&comefrom=weixin";
				header ( "location:$appurl" );
			}
		}
        ...
        $bestanswer = $this->vip_member_model->nickname($question,false,'answer');
	...
	function loadinavterbyanswerincid() {
        ...
        $userlist = $this->vip_member_model->nickname_username($userlist,true,'question');
	...
	function ajaxadopt() {
		...
		// 极光推送
		$url = url ( "question/view/$qid" );
		jpushmsg ( "您得回答已被作者采纳为最佳答案", $answer ['authorid'], "回答被采纳", $url );
...
// 			}
	...
	function edit() {
		...
		if (null !== $this->input->post ( 'submit' )) {
			...
			$this->db->wehre(array('id'=>$qid))->update('question',$datapos);
			if ($tags != '' && $tags != null) {
				$taglist = explode ( ",", $tags );
				$taglist && $this->tag_model->multi_addquestion ( array_unique ( $taglist ), $qid, $cid, $this->user ['uid'] );
			}
	...
	function ajaxedit() {
		...
		if (null !== $this->input->post ( 'submit' )) {
			...
			$this->db->where(array('id'=>$qid))->update('question',$datapos);
			if ($tags != '' && $tags != null) {
				$taglist = explode ( ",", $tags );
				$taglist && $this->tag_model->multi_addquestion ( array_unique ( $taglist ), $qid, $cid, $this->user ['uid'] );
			}
