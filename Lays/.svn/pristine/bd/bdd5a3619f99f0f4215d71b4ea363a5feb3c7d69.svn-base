...
class Topic extends CI_Controller {
	...
	function search() {
		...
		$topiclist = $this->topic_model->get_bylikename ( $geshiword, $startindex, $pagesize );
      
		$rownum = $this->topic_model->rownum_by_title ( $geshiword );
	...
	function ajaxpostcomment() {
		...
		if ($id > 0) {
			...
			if ($state == 1) {
				...
				// 极光推送
				$url = url ( 'topic/getone/' . $tid );
				jpushmsg ( "您的文章[$title]有新评论", $topicone ['authorid'], "文章有新评论", $url );
				...
				// 极光推送
				$url = url ( 'topic/getone/' . $tid );
				jpushmsg ( "您的文章[$title]有新评论", $topicone ['authorid'], "文章有新评论【审核中】", $url );
	...
	function index() {
		...
		$topiclist = $this->topic_model->get_list ( 2, $startindex, $pagesize );
        $topiclist = $this->vip_member_model->nickname($topiclist,true,'topic');
	...
	*/
	...
	function getone() {
        ...
        $topicone = $this->vip_member_model->nickname($topicone,false,'topic');
		...
		$newct=str_replace('<pre class="brush:php;toolbar:false">', '<pre ><code >', $newct);
