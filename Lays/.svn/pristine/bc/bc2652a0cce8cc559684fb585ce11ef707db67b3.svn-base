...
class Category_model extends CI_Model {
	...
	/**
	  * 通过一个分类ID 找到 下级 所有分类ID
	  * $srchcategory 一级分类
	  * $category2 二级分类
	  * $category3 三级分类
	  **/
    function get_class_id($srchcategory,$category2,$category3){

    	  $class_array_id = [];
    	   if($category3){
			   $class_array_id[] = $category3;
		   }else if($category2){ //通过 二级分类ID 获取所有的三级分类ID
			   $class_All  =  $this->get_js('','',1);
			   $class_array_id[] = $category2;
			   foreach ($class_All['category3'] as $key=>$val){
                              if($val[0] == $category2){
                                           $class_array_id[] = $val[1];
							  }
			   }
		   }else {   //通过一级分类ID 获取所有二级分类ID、以及二级分类下面的所有ID
			   $class_All  =  $this->get_js('','',1);
			   $class_array_id[] = $srchcategory;
			   foreach ($class_All['category2'] as $key=>$val){
				   if($val[0] == $srchcategory){
					   $class_array_id[] = $val[1];
				   }
			   }
			   foreach ($class_All['category3'] as $k=>$v){
				   if(in_array($v[0],$class_array_id)){
					   $class_array_id[] = $v[1];
				   }
			   }

		   }
    	   return $class_array_id;
	...
	function listtopic($status, $start, $limit) {
		...
		$wherecondition=" and onlybackground!=1 ";
	...
	function get_categrory_tree($type = 1) {
       global $category;
		...
		foreach ( $allcategory as $key => $category1 ) {
			if ($type == 1) {
				if ($category1 ['pid'] == 0 && $category1 ['isuseask'] == 1) {
			...
			if ($type == 2) {
				if ($category1['pid'] == 0 && $category1['isusearticle'] == 1) {
					$categrorytree .= "<option value=\"{$category1['id']}\">{$category1['name']}</option>";
					$categrorytree .= $this->get_child_tree ( $allcategory, $category1['id'], 1 );
				}
			}
...
$uid = intval ( $uid );
	...
	function list_by_name($name, $start = 0, $limit = 10) {
		global $user;
		$categorylist = array ();
	$query=$this->db->order_by("followers DESC")->like('name',$name)->limit($limit,$start)->get('category');
	...
	*/
	...
	function get_js($cid = 0, $type = 0,$is_array = 0) {
		$cid = intval ( $cid );
		$categoryjs = array ();
		...
		if($is_array){
			...
			foreach ( $query->result_array () as $category ) {
				switch ($category ['grade']) {
					case 1 :
						$category1[] = array($category ['id'],$category ['name']);
						break;
					case 2 :
						$category2[] = array($category ['pid'],$category ['id'],$category ['name']);
						break;
					case 3 :
						$category3[] = array($category ['pid'],$category ['id'],$category ['name']);
						break;
				}
			}
			$categoryjs ['category1'] = $category1;
			$categoryjs ['category2'] = $category2;
			$categoryjs ['category3'] =  $category3;
		}else{
			foreach ( $query->result_array () as $category ) {
				switch ($category ['grade']) {
					case 1 :
