	...
	function get($id) {
		...
		if ($question) {
			...
			if ($question ['hidden'] == 1) {
				...
				$question['user']['gender']="保密";
				$question['user']['signature']="保密";
			} else {
		 ...
		 if($question['user']){
			 $question= $this->vip_member_model->nickname($question,false,'question');
			 $question['user'] = $this->vip_member_model->nickname_username($question['user'],false,'question');
		 }
 ...
 function getallnosolvequestion(){
	...
	function search_title($title, $status = '1,2,6', $addbestanswer = 0, $start = 0, $limit = 20) {
		...
		if ($this->base->setting ['xunsearch_open']) {
			...
			$query=$this->db->where_in('status',$status)->like('title',$title)->or_like('description', $title)->limit($limit, $start)->get('question');
