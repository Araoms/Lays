...
class Topic_model extends CI_Model {
	...
	function get($id) {
		...
		if ($topic) {
			...
			$topic ['describtion'] = checkwordsglobal (stripslashes( $topic ['describtion']) );
		}
		return $topic;
	}
	/* 获取某个文章信息 */
	...
	function rownum_by_search($title = '', $author = '', $cid = 0) {
		...
		if ($cid) {
			$condition .= " AND `articleclassid" . "`in $cid ";
		}
	...
	function get_bycatid($catid, $start = 0, $limit = 6, $questionsize = 10) {
		$catid=intval($catid);
		$topiclist = array ();
		$query = $this->db->query ( "SELECT * FROM " . $this->db->dbprefix . "topic where articleclassid in($catid) and state=1  order by id desc LIMIT $start,$limit" );
		foreach ( $query->result_array () as $topic ) {
		...
		foreach ( $query->result_array () as $topic ) {
			if ($topic ['articleclassid'] > 0) {
				...
				if ($topic ['price'] > 0) {
					...
					$topic ['images'] = getfirstimgs ( $topic ['describtion'] );
					$topic ['describtion'] = clearhtml ( $topic ['describtion'] );
		...
		if ($query) {
			foreach ( $query->result_array () as $topic ) {
				...
				if ($topic ['price'] > 0) {
					...
					$topic ['describtion'] = clearhtml ( $topic ['describtion'], 100 );
				}
	...
	function updatetopic($id, $title, $desrc, $filepath = '', $isphone = '', $views = '', $cid, $ispc = 0, $price) {
		...
		if ($setting ['loaclimage']) {
			$desrc = replace_imgouter ( $desrc );
			$desrc = filter_otherimgouter ( $desrc );
			$filepath = getfirstimg ( $desrc );
		}
		$state = 1;
	...
	function addtopic($title, $desc, $image, $author, $authorid, $views, $articleclassid, $price = 0, $readmode = 1, $freeconent = '') {
		...
		$this->user_model->up_articles($authorid);
		return $aid;
	}
	...
	function remove($tids) {
		...
		$this->db->where_in('typeid',$tids)->where(array('itemtype'=>'article'))->delete('tag_item');
	...
	function vertify($tids) {
		$this->db->where_in('id',$tids)->update('topic',array('state'=>1));

	}
	/* 后台管理移动分类顺序 */
	function order_topic($id, $order) {
		$this->db->where_in('id',$id)->update('topic',array('displayorder'=>$order));
	...
	function makeindex() {
		if ($this->base->setting ['xunsearch_open']) {
			...
			foreach ( $query->result_array () as $topic ) {
				...
				$topic ['readmode'] = $topic ['readmode'];
				$topic ['freeconent'] = $topic ['freeconent'];
	...
	/**
	  ...
	  * $class_id
	  **/
	function get_topic_all($class_id= [],$filed= '*'){
		  $sql = "SELECT $filed FROM `" . $this->db->dbprefix . "topic`  WHERE `articleclassid` in " . $class_id;
	...
	function update_all($data){
		 ...
		 $sql = "UPDATE  `" . $this->db->dbprefix . "topic`  SET  describtion = CASE id ";
         foreach ($data as $key=>$val){
         	 $describtion = addslashes($val['describtion']);
			 $sql.="WHEN " .$val['id']." THEN "."\"$describtion\"";

		 }
		$sql.=" END WHERE id IN ".'('.implode(',',array_column($data,'id')).');';
