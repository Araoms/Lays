...
class Tag_model extends CI_Model {
	...
	*/
	...
	function gettaglistbycid($cid, $start = 0, $limit = 11) {
		...
		$query = $this->db->query ( "select distinct at.id,at.tagname,at.tagalias from " . $this->db->dbprefix . "tag as at," . $this->db->dbprefix . "tag_item as ati where ati.tagid=at.id and ati.cid=$cid order by at.tagquestions desc  LIMIT $start,$limit" );
	...
	*/
	function remove_by_id($ids) {
		$namestr = "'" . implode ( "','", $ids ) . "'";
		$this->db->query ( "DELETE FROM " . $this->db->dbprefix . "tag WHERE `id` IN ($namestr)" );
	   
	    //删除标签相关问题
		$this->db->query ( "DELETE FROM " . $this->db->dbprefix . "tag_item WHERE `tagid` IN ($namestr)" );
