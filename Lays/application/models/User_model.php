...
class User_model extends CI_Model {
	...
	function adduserapi($username, $password, $email = '', $groupid = 7, $uid = 0, $phone = 0,$memberIdx = 0) {
		$password = md5 ( $password );
		if ($uid) {
			$data = array (
					'uid' => $uid,
	...
	function up_articles($id){
        $sql = "UPDATE `{$this->db->dbprefix}user` SET articles = articles+1 WHERE uid = ".$id;
     ...
     * 获取多条数据
     */
    function getAll($where=array(),$field='*',$limit='',$group='',$where_in ='',$where_in_array=array()) {
     ...
     * 获取单条数据
     **/
    function get($where=array(),$field='*',$group='',$like='') {
