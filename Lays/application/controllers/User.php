...
class User extends CI_Controller {
    ...
    public function __construct() {
        $this->whitelist = "usernotify,home,invatelist,myjifen,invateme,search,spacefollower,vertifyemail,creditrecharge,vertify,editemail,editphone,sendcheckmail,space_attention,userzhangdan,userbank,postrequestmoney,getsmscode";
        parent::__construct ();
    ...
    function getpass() {
        ...
        if (null !== $this->input->post ( 'submit' )) {
            ...
            $name =addslashes( $this->input->post ( 'username' ));
            ...
            if ($touser) {
                ...
                sendmailto ( $email, $subject, $message, $name );
    ...
    function resetpass() {
        ...
        if (null !== $this->input->post ( 'submit' )) {
            ...
            if ($this->setting ["ucenter_open"]) {
                $this->load->model ( 'ucenter_model' );
                $this->ucenter_model->uppass ( $user ['username'], '', $password, $user ['email'], 1 );
            }
    ...
    function ask() {
        ...
        if($questionlist){
            ...
            $questionlist = $this->vip_member_model->nickname($questionlist,true,'question');
        }
    ...
    function answer() {
        ...
        if($answerlist){
            $this->load->model ( 'vip_member_model' );
            $answerlist = $this->vip_member_model->nickname($answerlist,true,'answer');
        }
    ...
    function attention() {
        ...
        if ($attentiontype == 'article') {
            ...
            if($topiclist){
                $topiclist = $this->vip_member_model->nickname($topiclist,true,'topicl');
            }
            ...
            if($attentionlist){
                $attentionlist = $this->vip_member_model->nickname_username($attentionlist,true);
            }
    ...
    function score() {
        ...
        if($doinglist){
            $this->load->model ( 'vip_member_model' );
            $doinglist = $this->vip_member_model->nickname($doinglist,true,'doing');
        }
        ...
        foreach ($usergroup as $key=>$val){
            if($val['groupid'] >7){
                if($this->user['credit1'] >=$val['creditslower'] && $this->user['credit1'] < $val['creditshigher']){
    ...
    function space() {
        ...
        if ($member) {
            ...
            foreach ($usergroup as $key=>$val){
                if($val['groupid'] >7){
                    if($member['credit1'] >=$val['creditslower'] && $member['credit1'] < $val['creditshigher']){
                        $title = $val['grouptitle'];
                    }
    ...
    function record(){
        ...
        $where = array('authorid'=>$this->user['uid']);
        $record_info = $this->record_model->getAll($where,'','','','id desc');
        @$page = max ( 1, intval ( $this->uri->segment ( 3 ) ) );
        $pagesize = $this->setting ['list_default'];
        $startindex = ($page - 1) * $pagesize;
        $departstr = page (count($record_info),$pagesize,$page,"user/record" );
