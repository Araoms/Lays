...
class Admin_topic extends CI_Controller {
    ...
    function index($msg = '', $ty = '') {
        ...
        if ($this->uri->rsegments [5] != '' && $this->uri->rsegments [5] != '0') {
            ...
            $class_id_attr = '';
            $srchcategory = $this->input->post ( 'srchcategory' );  //一级分类ID
            $category2 = $this->input->post ( 'category2' );        //二级分类ID
            $category3= $this->input->post ( 'category3' );         //三级分类ID
            if($srchcategory||$category2||$category3){
                //由于文章只存了一个ID 不知道是一级分类 还是 二 、三级 只得查出它上级
                $class_id_attr =  $this->category_model->get_class_id($srchcategory,$category2,$category3);
                $class_id_attr =  '('.implode(',',$class_id_attr).')'; // (1,2,3,)
            }
        }

        $topiclist = $this->topic_model->list_by_search ( $srchtitle, $srchauthor, $class_id_attr, $startindex, $pagesize );

        $rownum = $this->topic_model->rownum_by_search ( $srchtitle, $srchauthor, $class_id_attr );
        if (! $srchtitle) {
            $srchtitle = 0;
        }
        if (! $srchauthor) {
    ...
    //文章批量实时替换 内容文字
    function rule_replacement(){

        $categoryjs = $this->category_model->get_js ();
        if($this->input->get('submit')){
            $rule = $this->input->get('rule');
            $rule_value = $this->input->get('rule_value');

            $replace_value = $this->input->get('replace_value');
            $replace = $this->input->get('replace');

            if(!($rule && $rule_value && $replace_value)){
                exit(json_encode(['status'=>0,'msg'=>'请填写要替换的 内容']));
            }
            $pregAll = array_combine($rule,$rule_value);
            $srchcategory = $this->input->get ( 'srchcategory' );  //一级分类ID
            $category2 = $this->input->get ( 'category2' );        //二级分类ID
            $category3= $this->input->get ( 'category3' );         //三级分类ID
            if($srchcategory||$category2||$category3){
                //由于文章只存了一个ID 不知道是一级分类 还是 二 、三级 只得查出它上级
                $class_id_attr =  $this->category_model->get_class_id($srchcategory,$category2,$category3);
                $class_id_attr =  '('.implode(',',$class_id_attr).')'; // (1,2,3,)
                $topic_class_all = $this->topic_model->get_topic_all($class_id_attr,'id,describtion');
                if($topic_class_all){
                    foreach ($topic_class_all as $key=>$val){
                        if(trim($replace_value)){
                            if($replace == 1){
                                $val['describtion'] = substr($val['describtion'],strripos($val['describtion'],$replace_value));
                            }else{
                                $val['describtion'] =  substr($val['describtion'],0,strrpos($val['describtion'],$replace_value)  + strlen($replace_value) );

                            }
                        }
                        foreach ($pregAll as $k=>$v){
                            if(trim($k)){
                                $val['describtion'] = preg_replace ( "($k)", $v?:'', $val['describtion']);
                            }

                        }
                        $topic_class_all[$key]['describtion'] = $val['describtion'];
                    }
                    $res =  $this->topic_model->update_all($topic_class_all);
                    if($res){
                        exit(json_encode(['status'=>1,'msg'=>'修改完成，本次修改成功'.count($topic_class_all).'条数据']));
                    }else{
                        exit(json_encode(['status'=>0,'msg'=>'修改失败']));
                    }
                }else{
                    exit(json_encode(['status'=>0,'msg'=>'此分类下面没有文章']));
                }
            }else{
                exit(json_encode(['status'=>0,'msg'=>'请选择分类']));
            }
        }else{
            include template('rule_replacement','admin');
        }
